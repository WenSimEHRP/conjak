...
#[wasm_export]
pub fn sep_by_ten_thousands(input: &[u8]) -> std::result::Result<Vec<u8>, String> {
    inner_sep_by_ten_thousands(input).map_err(|e| e.to_string())
}

fn inner_sep_by_ten_thousands(input: &[u8]) -> Result<Vec<u8>> {
